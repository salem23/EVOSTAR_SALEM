...
%TORCS + NN
Neural networks have been applied successfully in recent years. Using supervised methods it is possible to create better controllers imitating an existent bot \cite{CardamoneImitation09}. However, performance decreases when using circuits not used for training. Human data can also be used for training the networks, for example to model a human-like controller \cite{MunozHumanLike10}. Although several scripts can be used to avoid noise, the problem to create a human-like driver is difficult, and, as in the previous work, the performance in circuits not used for training may be reduced.
%TORCS+EA
Evolutionary algorithms have been applied to optimize the hand-coded strategies parameters \cite{ButzCMAES09}, optimizing several modules, and creating bots that can win competitions.

%FUZZY
Fuzzy logic has been used widely since the year 2008. Authors of \cite{Fujii2008} had examined the performance of fuzzy rule-based systems in  high-level decision making of a car and the effect of sensory information. 
...
Some approaches are based on context-dependant hybrid fuzzy-sets, avoiding training and dealing with input variables \cite{ThangCIG08}. Authors claim that using Machine Learning techniques and fuzzy approaches may reduce the  training time. %PABLO: ojo, no usa TORCS

%FUZZY + EAs
Genetic algorithms have been also used to optimize the fuzzy set parameters, although the maximum values of the sets may not be achieved \cite{PerezEvolvingFuzzy09}. In \cite{Guadarrama2008}, a fuzzy rule-based car controller for Car Racing Competition was built and tuned with coevolutionary genetic algorithms. Indeed, two fuzzy controllers were designed: the first one to compute the acceleration using the velocity and distance and the second one to get the turning angle. Their results had been compared with cooperative and competitive approaches to tuning parameters.
 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The car considered is \textit{car1-tbr1} of SCR 1 Server, which is a NASCAR car part of the SCR Server team. It has a weight of 1150 KG, max fuel: 94 KG, length= 4.25m and a width of 1.94m. 
...
\item AD5: Fuzzy speed controller that takes the opponent into account.
