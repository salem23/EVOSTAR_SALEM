...
This fuzzy controller has three input values and one output: the target speed (see Figure \ref{AD}).
...
\end{figure}
%
The automatic driver (AD) presented in that figure is a Mamdani-based fuzzy system \cite{iancu2012} with trapezoidal Membership Functions (MF) for input variables,  because it avoids sudden changes in input values.
...
The values for S1 to S4 are respectively: [0, 0.25, 0.5, 1]
when M10=track[7] we will take negative values of the steer (sterr=-steer)
%Mohzammed--- habia un error en las reglas, he metido los mismos que las del speed
% Antonio - TODO: poner los valores en orden
...
In this section we present all the experiments that we performed in order to measure the performance of our AD controller.
...
%We have established a set of tests to measure the performance of the controller AD. 
%salem -automatic driver , ya le he metido
% Antonio - ¿Qué significa AD? No se ha dicho antes.
...
\end{itemize}
% Mohammed lo es,  Antonio 
% Antonio - he escrito la justificación de las pistas según he creido que se hizo. A ver si es así, Mohammed. ;D
...
% Antonio - Ok, I have commented this before. Check it out.
\end{table}

The aim of this selection is to test the value of the controller in a wide set of situations, in order to check its potential as a global controller.
...
In order to test the different sub-controller features in a separated way
to see which one has the most influence on the success or failure of
the controller, we have tested them In order to validate the proposed fuzzy controllers, we have considered the following bots in the experiments.
...
We firstly test our controllers in an easier scenario/race, we considered a 20 laps practice race in the five tracks to check the behaviour of the proposed controllers. We considered 20 as it is the maximum number of laps a car can perform without doing a pit stop, because our controller is not prepared to plan and manage this event. 
% salem - I choose 20 laps because we have to test them in a long race and 20  is enough , no in any case, I can change it
...
five tracks. This was due to the steering unit of the controller. 
% salem - Late braking  and maintaining a maximal speed -with the steer controller- will allow us to follow optimal pathes and earn race time.
% what
...
the tracks safely except the Dirt1 and Forza circuit where the damage
...
Results are in Table \ref{resulta9} where it could be seen that the the AD5  controller has given good results in th e oval and road tracks but in the Dirty one was a disaster, it suffers from damage and the TORCS  simulator had to stop it.
This is due to the lack of grip in this kind of tracks and the damages are caused by the shocks with the track border due to a dysfunction
 of the steer module.
% Mohammed, Done well I think so
 % YOu have to comment the
...
In this context we tested "AD5" in 5 tracks against berniw, bt, damned, inferno and tita teams which are integrated TORCS controllers with different strategies and slighter modifications. Also , bt controller comes with advance machine learning \cite{oponnents2010}. 
%Mohammed ,  hecho
