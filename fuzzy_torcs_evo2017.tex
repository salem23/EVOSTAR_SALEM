...
three-dimensional racing video game created by Eric Espié and
...
decisions in the race). Indeed, several researchers have taken advantage of this technique to design TORCS controllers, for example \cite{Guadarrama2008,CarRacing_Pelta09}. 
% Antonio - TODO: poner más citas de controladores difusos
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  TORCS 
In the last years, several researchers have focused on driving a racing car using computational intelligence techniques \cite{evol},\cite{torcs2012}, \cite{neurone}.  %and especially fuzzy logic where \cite{CarRacing_Pelta09} tried to design a fuzzy controller for the the target speed while the other car actions were obtained by simple functions while in\cite{ho2008}, a  previous fuzzy controller was enhanced using context-dependent fuzzy set, the obtained controller was compared to existing driving techniques showing better performance. %PABLO: estos papers han sido comentado ya en la sección fuzzy (ho2008 lo he re-leido yo :/)

%TORCS + NN
Among them, Artificial Neural Networks (ANN) have been one of the most prolific techniques applied successfully to this problem. For instance, using supervised methods it is possible to create controllers by the imitation of an existing bot \cite{CardamoneImitation09}. However, their performance decreases if they drive on circuits not previously used for training. 

Human data can also be used for training these networks, or for example, to model a human-like controller \cite{MunozHumanLike10}. Although several scripts can be used to avoid noise, the problem to create a human-like driver is difficult, and again, their performance in circuits not known will be reduced.

%TORCS + EA
These models can be improved using meta-optimisation techniques, such as Evolutionary Algorithms (EAs). These have been applied to `refine' the parameters \cite{ButzCMAES09} considered in the different methods (being them hand-coded or not); optimising then controllers' behavioural modules, and yielding bots that can even win competitions [REFs].
...
In \cite{CarRacing_Pelta09}, a fuzzy controller was designed to obtain the target position for the car, whereas the steering was computed using a crisp formula. This work and its improvement \cite{torcs2012} have some common points with our proposal, as the controller is pretty similar, however in this work two different specialised fuzzy modules have been implemented instead of just one.

Some approaches are based on context-dependant hybrid fuzzy-sets, avoiding training and dealing with input variables \cite{ThangCIG08,ThangG08}. Authors claim that using Machine Learning techniques and fuzzy approaches may reduce the  training time. %PABLO: ojo, no usa TORCS

%FUZZY + EAs
Genetic algorithms have been also used to optimise the fuzzy set parameters, although the maximum values of the sets may not be achieved \cite{PerezEvolvingFuzzy09}. In \cite{Guadarrama2008}, a fuzzy rule-based car controller for Car Racing Competition was built and tuned with co-evolutionary genetic algorithms. Indeed, two fuzzy controllers were designed: the first one to compute the acceleration using the velocity and distance and the second one to get the turning angle. Their results had been compared with cooperative and competitive approaches to tuning parameters.
...
\begin{table}[ht!]
	...
	\begin{tabular}{|p{3cm}|p{3 cm}|p{6 cm}|}
		...
		\hline
	\end{tabular}
	\caption{TORCS Actuators \cite{torcs_Onieva2011}}
	\label{tab2}
\end{table}
...
%Mohammed -because the actions are not computed once and in serial mode but some modules are called several times like the steer, stuck module and abs
...
designed controllers \cite{torcs_Onieva2011,torcs2}. It presents very basic
...
The set of sensors was chosen considering those used in previous works \cite{torcs_Onieva2011}, but refining it after an exhaustive systematic experimentation phase. Thus, the final set of sensors are those which yielded the best results in a wider amount of tracks.
...
%Mohammed si, el cont  de steer  tiene las mismas entradas que el de speed pero las reglas son differentes
% Antonio - He supuesto esto. ¿Es cierto, Mohammed? :D
...
% Mohammed, the values of steer in TORCS  are from 1 TO -1   from left to right , I don't know why  
% M10 will be equal to Max (Track[7], Track[11])


% Antonio - Si M10 = Track[7] entonces la curva será a la derecha, ¿no? ¿Entonces el giro será negativo o positivo?
% ¿No ocurriría lo mismo si M5 = Track[8]?
...
The car considered is \textit{car1-tbr1} of SCR 1 Server, which is a NASCAR car part of the SCR Server team. It has a weight of 1150 KG, max fuel: 94 KG, length= 4.25m and a width of 1.94m with 300km/h as maximal speed.
%Mohammed done, it is not the fastest car , I wanted to test our controller using a medium car so the obtained perf are not from the car but from the controller
% Antonio - ¿Cuál es la velocidad MAXSPEED del coche?

% Why this one? Does that mean that
% results apply only to this kind of
% car? - JJ

% Antonio - ¿Es un coche típico de las competiciones?¿Tiene alguna propiedad que lo hace interesante/mejor que otros?
...
\begin{itemize}
\item AD1: Fuzzy speed controller. 
\item AD2: Fuzzy speed and steering controller.
\item AD5: Fuzzy speed  and steering controller that takes the opponent into account.
%Mohammmed se llama 5 porque hemos borrdo el 4  que se basa sobre otros sensores (Turning radius) debemos cambiar su nombre , no?
%considerar el oponente en qué consiste en tener en consideracion los sensores opponent que nos da la distancia de los otros corredores
% etstos sensores nos dan la posicion e
% Antonio - Es una tontería, pero ¿por qué se llama '5'?
% ¿Esto de considerar el oponente en qué consiste? Habría que explicarlo. 
% ¿Es algo nuevo de este artículo?
\item SD: Simple driver.
\end{itemize}

Since the speed controller is similar to previous approaches, it was expected to work properly. Hence, we have checked the basic version of our controller AD1 and compared it with modified/improved versions, such as AD2 and AD5.
As stated before, SD has been taken as a baseline for comparisons.
%Mohammed , si es verdad
% hemos empezado con el de speed   AD1 despues hemos integrado el cont de steer  AD2  y enfin el AD5  que debe llamrse AD3
% Antonio - Mohammed, revisa esto a ver si es verdad. ;)
...
%  Mohammed , hemos lanzado cada cont de los tr  solo en una carrera sin rivales  
% Antonio - esto lo he supuesto, pero no sé si 'practise races' son como digo: sin rivales
We considered a 20 laps in the five tracks, aiming to check if the controllers are able to perform well in these `warm up' races.
We considered 20 as it is the maximum number of laps a car can perform without doing a pit stop, because our controller is not prepared to plan and manage this event. 
...
\multicolumn{5}{c}{\textbf{CG-Speedway Number1}}\\	
	...
	\label{Result1r}
\end{table}
% Mohammed , minspeed is negative due to braking
% fuel is by kg 
% Antonio - Algunas cosas a revisar:

% - Hay velocidades mínimas negativas
% - ¿Qué valor es el de Fuel?¿Es un tiempo?
...
The target position 
%Mohammed- target position= next (target) steer computed by the fuzzy cont 
% Antonio - ¿'target position' de qué?¿del coche?¿de los rivales?¿hay rivales en estas carreras?
allows the car to safely adjust the required steering angle with the maximum speed so the controller AD5 gave the best time.
% Mohammed , lo tiene , he corigedo eso arriba 
% Antonio - Pero AD5 no tiene steering control, ¿no? O no se dijo en la descripción de los conrtroladores AD1, AD2 y AD5.

However, taking a smaller target angle required a greater distance to
cover by car which follows a less efficient trajectory. In addition, the
...
of the track when the car goes out. The simple SD driver
finished the race without damage, while AD5 was able to complete all
the tracks safely except the Dirt1 and Forza circuits, where the damage
...
% Mohammed shocks with the track border which causes damages
% Antonio  ¿Qué son 'shocks rate'?

AD2 gets quite bad results, even in the comparison with the simple driver...
% Antonio - TODO: comentar esto, ¿por qué pasará? :_(
...
In the second experiment, the race stopping criterion is set to 300s, so, after that time, the simulation stops. Once the race has ended, the distances covered by each controller are checked.

The results are shown in Table \ref{resulta9} where it could be seen that the the AD5 controller has yielded the best results in the oval and road tracks. However, in the Dirty one, it was a disaster, since it was extremely damaged and TORCS simulator had to stop it.
This happened to the lack of grip in this kind of tracks and the damages which were caused by the crashes/collisions with the track border. The reason might be a dysfunction of the steer module.
% Mohammmed - si en este circuito, el coche sale muchisimas veces de la pista en curvas y lo que controla esto es el steer module 
% Antonio - ¿te refieres a un error inesperado o a que no está bien diselado el controlador?
...
\begin{table} 
	...
	\begin{tabular}{ |p{3cm}|p{3cm}|p{3cm}|p{3 cm}|}	
		...
		\\
		\hline
		AD1 & 04:49:12 & 7825.032&126  
		\\
		\hline
		AD2 & 03:45:02 &5202.99& 134
		\\
		\hline 
		AD5 & 01:36:14 &4620.25& 153 
		\\
		\hline 
		...
		\\
		\hline
		AD1 & 02:49:12  & 17352.30  &  247
		\\
		\hline
		AD2 & 02:49:12  & 17354.12 & 239
		\\
		\hline 
		AD5 & 57:66 & 17355.20 & 203 
		\\
		\hline 
...
Finally, a real race test has been conducted. Our main goal is to test the performance of AD5 in a race, and answer the following questions: Could we have a perfect driving in a race with AD5 only with the track borders sensors? Can we win the race with AD5?

In this context we tested AD5 in 5 tracks against \textit{berniw, bt, damned, inferno} and \textit{tita} teams which are integrated TORCS controllers with different strategies and slightly modifications. Moreover, \textit{bt} controller comes with advanced machine learning methods \cite{oponnents2010}.

%Mohammed son controladores distribuidos con TORCS  y utilizados por Pelta para comprar su  cont, es por eso que he los  considerado
...
After the launch of the race, AD5 against the 10 cars of each team for 20 laps in each track we obtained the results presented in Table \ref{resultat31}.
...
\end{table}
% Mohammed, he quitado los otros por el espacio y porque los con el dirty no son buenos y nuestro cont no termina la carrera
% Antonio - faltan los resultados de 3 pistas. Sólo están Forza y E-Track5
...
According to damage results, despite our controller can win the race on the E-track5, it had a higher crash rates in several cases. We observed also that in the majority of cases our car is very slow compared to the other cars in each team. 

The obtained results are due to the optimal trajectory in curves and braking as late as possible.
In E-track5, the controller has win the race because the used track has different curves so the car can follow an optimal path given by the steer module. Furthermore, the target speed sub-controller allowed to go as faster as possible. However, in the Forza track which is characterised by a small width and less curves, the drawbacks of the proposed controller had clearly affected the results.
...
%salem -  the strength of our controller is in detecting curves and late braking so it does not waste time in turning, to verify this, we run them in different tracks 
% Antonio - podemos decir esto aquí
%Mohammed - no sé, podemos construir un cont por estos cinco tracks pero por todos , no lo pienso, podemos rafinar los parametros por cada typo 
% Antonio - decir que, de acuerdo a los resultados, se podría obtener un controlador 'universal' o 'global' refinando los parámetros con algún tipo de optimización (EAs)
