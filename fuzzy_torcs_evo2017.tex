...
\title{MascaFuzzy: driving in TORCS using fuzzy controllers}
...
within a limited budget. In this paper, we present the design of a
driver for racing car in a simulated race. Unlike previous
controllers, that only used fuzzy controllers for either accelaration
or steering, the proposed driver uses
two fuzzy controllers for steering and computing the target speed of the
car using the track border sensors as inputs; besides, for enhanced
safety, we have also taken into account the position of the other
competitors.
The  fuzzy controller is evaluated in practise and timed races
...
decisions in the race). Indeed, several researchers have taken advantage of this technique to design TORCS controllers, such as \cite{Guadarrama2008,CarRacing_Pelta09}. 
...
designed controllers \cite{torcs,torcs2}. It presents very basic
...
The first proposed fuzzy sub-controller aims to estimate the optimal target speed of the car in straight parts and curves of the rack where the main goal is to go as faster as possible and secure the car.
%salem- done; well, I think
...
% salem - done by the simple driver  and maintained by our controller
...
The obtained target speed will be used for computing the value of acceleration.
%salem- 
...
\begin{figure}
	...
	\label{AD}
\end{figure}
The fuzzy driver (FD) presented in that figure is a Mamdani-based fuzzy system \cite{iancu2012} with trapezoidal Membership Functions (MF) for input variables  because it avoids sudden changes in input values.
% salem- 
...
% Antonio - Revisar los valores, ¿estan bien los de M10? Low está incluido en Medium...
	...
	The base of rules is designed so that if the frontal distance is maximal, then the target speed should be the maximum. Its value should be lower when the frontal distance is shorter. Thus, the fuzzy rules are listed below:
		% salem he obtenido estas reglas imitando el comportamiento de un conductor
...
% Antonio - ¿Cómo se han obtenido estas reglas? ¿De forma manual o se han generado con alguna técnica?¿Se han descartado otras?
	...
	\end{itemize}
MAXDISTSPEED is the a longest track sensors possible value and MAXSPEED is a car variable of the maximal car speed, in case of car-trb1 model, MAXSPEED=300.
% Antonio - aclarar esto

The output value is encoded by seven singletons [30, 60,120,180,220, 240, 280].
...
In order to detect the curves, the controller focuses on the sensor values (M10, M5, and Front).
 % salem si  el sensor de frente es mas grande , hay linea recta, si los de derecha ( M10 Y M5  hay curva a la derecha)
...
%salem -  the strength of our controller is in detecting curves and late braking so it does not waste time in turning, to verify this, we run them in different tracks 
  ...
  % What do those tests imply? Are the like the
...
% salem, no it's not a global controller, but we must check if it works in different tracks
% to do that, we could optimize the parameters in future works 
...
%salem- previous controllers have computed the speed with fuzzy but not the steer, is what our controller do, 
...
%salem- width of the track, steer is computed reffering to this 
...
% salem , he borrado eso 
% Antonio - ¿Por qué se comenta esto?¿Es importante?

In our simulations, we used the car 'car1-tbr1' which has a weight of
1150 KG, max fuel: 94 KG, length= 4.25 m and a width of 1.94 m. 
% salem, no , bu we have to choose a car  the MAXSPEED  will get a concrete value
% Why
...
We firstly test our controllers in an easier scenario/race , we considered a 20 laps practice race in the five track to check the behaviour of the proposed controllers.
...
five tracks. This was due to the steering unit of the target. 
...
\begin{table} 
	...
	\end{tabular}
\end{table}
% salem- comments added
From the Table \ref{resulta9}, the AD5 controller has obtained the best time in the E-Track5 oval track  and in road tracks but it was a disaster in dirty tracks where it damages the car leading to a small covered distance and 
...
In this context we tested "AD5" in 5 tracks against berniw, bt, damned, inferno and tita teams which are integrated TORCS bots with different strategies and slighter modifications \cite{oponnents2010}.
%salem - done
...
The obtained results are due to the optimal trajectory in curves and braking as late as possible. Indeed, the proposed controller has given best ranking against powerful TORCS teams in the oval track but in the other one, it shocks with the track borders which has led to more damage rate.   
