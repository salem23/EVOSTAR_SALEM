...
% Mohammed in curves, M10 is greater then M5
% Antonio - ¿No ocurriría lo mismo si M5 = Track[8]?
...
\item AD-SPSC: Fuzzy speed and steering controller.
\item AD-SSOP: Fuzzy speed and steering controller that takes the opponent into account. Thus, it considers the sensors that give the distance from the opponents to the car.
% Mohammed en este caso, si el sensor del opponente que esta en el 10° nos daba un valor differente de 0m , M10 = opponenttrack  (sensor give distance to the botder while opponent give distance to each opponent of the 36  around the car )
% Antonio - ¿cómo se incluyen estos sensores?¿hay nuevas reglas?¿se cambian las que hay?¿afectan a la velocidad o al steering?
\item SD: Simple driver.
\end{itemize}

Since the speed controller is similar to previous approaches, it was expected to work properly. Hence, we have checked the basic version of our controller AD-SP and compared it with modified/improved versions, such as AD-SPSC and AD-SSOP.
...
%Mohammed -!!!!!
% Antonio - ¿Por qué AD-SSOP (AD5) obtiene mejores resultados que AD-SPSC (AD2), si no hay rivales y AD-SPSC también tienen Steering Control?
...
%Mohammed - correcto
% Antonio - Pero el Damage en Forza para AD-SSOP (AD5) es 0. ¿Está bien?

Regarding the Fuel consumption, it  depends on shocks rate and category of tracks. 
% Mohammed shocks with the track border which causes damages
% Antonio  ¿Qué son 'shocks rate'?
% Antonio - ¿El Fuel consumido depende de los choques con el borde de la pista?

AD-SPSC gets quite bad results, even in the comparison with the simple driver...
% Mohammed - he verificado los experimentos, los del AD-SPSC son resultados del control del steer solamente, Normalmente, no se comparan los dos en una carrera sin oppponentes . Que hago? les quito?
% Antonio - TODO: comentar esto, ¿por qué pasará? :_(

% Mohammed, ¿por qué crees que AD-SPSC, que tiene control de velocidad y de steering obtiene malos resultados? Si no hay rivales, ¿no debería se tan bueno como AD-SSOP?¿Por qué AD-SSOP es mucho mejor si no hay rivales???
...
In the second experiment, the race stopping criterion is set to 300s, so, after that time, the simulation stops. 
% Antonio - ¿Hay oponentes en la carrera?
...
AD-SPSC also showed problems in the same circuit so this lead us to reinforce the fact the fuzzy steering control module is too much sensitive to the bad conditions of the road. This is a point of improvement that we will address in a close future.
...
Finally, a real race test has been conducted. Our main goal is to test the performance of our best controller, AD-SSOP, in a race, and answer the following questions: Could we have a perfect driving in a race with AD-SSOP only with the track borders and opponent's sensors? 
%Mohammed - claro
% Antonio - También usará los sensores de los oponentes, ¿no? Si no ¿para qué usar AD-SSOP?
Can we win the race with AD-SSOP?
...
% Antonio - creo que habría que poner los otros resultados, aunque sean malos. 
% Es un poco raro omitir cosas por ser malas. ;)
% Se puede no poner  Dirt1 si el controlador no termina la carrera.
...
Furthermore, the target speed sub-controller allowed to go as faster as possible.
...
For instance, in the Forza track which is characterized by a small width and less curves, the drawbacks of the proposed controller had clearly affected the results.
...
This approach differs from others of the state of the art due to the combination of both sub-controllers, which is a novel variation.

The designed controller has been evaluated in five circuits with different shapes and conditions and considering three different scenarios: a practise race, a time trial race, and a real race against though opponents belonging to TORCS teams. Three different combinations of the fuzzy modules have been tested, namely one with the target speed computation (AD-SP), one with speed and steerig control (AD-SPSC), and one with both of them plus the consideration of the opponents (AD-SSOP).
...
In addition, in future work we also aim to improve the controller to design a smarter racing car driver. This could be enhanced by tuning its parameters using an evolution algorithm or/and combine it with a learning tool (neural networks) to recognize the track parts and plan in advance.
