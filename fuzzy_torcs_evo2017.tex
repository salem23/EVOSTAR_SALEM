...
In each simulation tick, the driver must make a decision to direct the car (steer) on the track and select a target speed, this problem is much harder when the car is near a curve where it has to make a compromise between go faster and secure the car to avoid slipping and go out of the track. The idea is to position  the car toward the farthest track border to avoid early breaking.\\
This complex and uncertain problem could be solved  by fuzzy logic due to its adaptive control and dealing with noise and uncertainties. Indeed, many researchers have token advantage of this technique to design TORCS controllers \cite{Guadarrama2008},\cite{neurone} so in \cite{CarRacing_Pelta09}, a fuzzy controller was designed to obtain the target position and the steer was computed through a crisp formula. 
% Salem - 
...
Thus, in this work, and in difference with \cite{CarRacing_Pelta09}, we present a novel fuzzy-based controller to drive cars in races inside TORCS. The proposed bot computes the steer and the target speed using two fuzzy sub-controllers, which consider the track sensors and the opponents' position in the race (track sensor to find the farthest border and opponents sensors for collusion avoidance and overtaking ).
% Salem - 
